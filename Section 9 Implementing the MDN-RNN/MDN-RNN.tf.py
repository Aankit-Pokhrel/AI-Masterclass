...
class MDN_RNN(object):
    ...
    def _build_model(self, hps):
        ...
        # We will make a series of functions b4 the loss function
        # first we will get the log of the sqrt of 2*pi
        # this is part of the loss function we will use to train the MDN-RNN
        logSqrtTwoPi = np.log(np.sqrt(2.0*np.pi))

        # We will compute the tf_lognormal
        def tf_logNormal(target, mean, logstd):
            # target: the target tensor
            # mean: the mean of the Gaussian distributions
            # logstd: the log of the standard deviation of the Gaussian distributions
            return -0.5 * ((target - mean) / tf.exp(logstd)) ** 2 - logstd - logSqrtTwoPi
            # tf.square: computes the square of the input tensor

        def get_lossFunc(logmix, mean, logstd, target):
            # logmix: the log of the mixture coefficients
            # mean: the mean of the Gaussian distributions
            # logstd: the log of the standard deviation of the Gaussian distributions
            # target: the target tensor

            v = logmix + tf_logNormal(target, mean, logstd)
        ...
        # Now we will get the loss function
        # self.output_x is in the shape (100,1000,32)
        flatTargetData = tf.reshape(self.output_x, [-1, 1])
        # flatTargetData: the flattened target tensor
        # shape: shape of the flattened target tensor, [-1, 1]
        # this turns a 3-D Tensor into a 2D Tensor with the shape of (3200000,1)

        # Calling the get_lossFunc
        lossFunc = get_lossFunc(out_logmix, out_mean,
                                out_logstd, flatTargetData)
        ...
        # checking if we are training
        if self.hps.is_training == 1:
            # Getting the Learning Rate
            self.lr = tf.Variable(self.hps.learning_rate)

            # Now we get the optimizer, using the adam optimizer
            self.optmizer = tf.train.AdamOptimizer(self.lr)

            # Now we compute the gradients
            # this is the same as the CNN-VAE
            gvs = self.optmizer.compute_gradients(
                loss=self.cost
            )
            # this gets the gradients and the variable pairs

            # Now we will clip the gradients to avoid exploding gradients
            # typically the clipping value is between -1.0 and 1.0
            ...
            self.train_op = self.optmizer.apply_gradients(
        ...
        # Final line of the _build_model function
